...
export const rolePunish = async (Client, users, time) => {
    console.log("punish him", users, time);

    let punishedUsers = JSON.parse(
                ...
                if (punishedUsers.includes(user.user.username)) {
                    ...
                    user.roles.add(
                        user.guild.roles.cache.find(
                            (r) => r.name == process.env.PUNISHMENT_ROLE
                        )
                    );
                    punishedUsers = [...punishedUsers, user.user.username];
                    fs.writeFileSync(
                        "./punishedUsers.json",
                        JSON.stringify(punishedUsers)
                    );
                    Client.channels.cache
                        .get(process.env.DISCORD_COMMAND_CHANNEL)
                        .send(
                            `${
                                user.user.username
                            } is abonished to the depths of hell 2.0 for ${parseInt(
                                time
                            )} minutes`
                        );
                    setTimeout(async () => {
