...
class lastJudgment {
    ...
    readDoomed(Client: discord.Client) {
        this.doomed = JSON.parse(
            fs.readFileSync("./punishedUsers.json", "utf-8")
        );
    ...
    checkRolesModyfication(
        roles: Array<string>,
        guild: discord.Guild
    ): boolean {
        return roles.some((role) => {
            const roleObj: discord.Role = guild.roles.cache.find(
                (r: discord.Role) => r.id == role
            );
    ...
    punishInit(Client: discord.Client, message: discord.Message, time: string) {
        ...
        Client.guilds.cache
            ...
            .members.cache.forEach((member: discord.GuildMember) => {
                this.user = member;
                if (users.has(this.user!.user.id)) {
                    if (
                        this.doomed.some(
                            (doomed: doomed) => doomed.id == this.user.user.id
                        )
                    ) {
                        console.log(`${this.user.user.username} już nie żyż`);
                        message.channel.send(
                            `${this.user.user.username} już nie żyż`
                        );
                        return this.user;
                    } else if (
                        isNaN(parseInt(time.slice(0, -1))) == false &&
                        typeof time[time.length - 1] == "string"
                    ) {
                        ...
                        if (
                            ...
                            message.channel.send(
                                `Cannot manipulate ${member.user.username} role`
                            );
                            return false;
                        } else
            ...
            this.doomed.some((o) => {
                if (o.id == user.user.id) {
                    ...
                    user.roles.member.roles
                        .add(
                            o.roles.filter(
                                (id) => id != process.env.SERVER_BOOSTER_ROLE_ID
                            )
                        )
    ...
    releaseDoomedFromFile(user: any, Client: discord.Client) {
        let savedRoles: json = JSON.parse(
            fs.readFileSync("./roles.json", "utf-8")
        );
        savedRoles.users.some((o: user) => {
            if (o.clientId == user.user.id) {
                console.log(
                    `zwalnianie użytkownika po pliku w którym istnieje`
                );
        ...
        user.roles
            .remove(
                userData.roles.filter(
                    (roleId: string) =>
                        roleId != process.env.SERVER_BOOSTER_ROLE_ID
                )
            )
            .then((lateruser: any) => {
                lateruser.roles
                    .add(
                        lateruser.guild.roles.cache.find(
                            (r: discord.Role) =>
                    ...
                    .then((afterAfterUser: any) => {
                        ...
                        const releasement: Moment = moment().add(
                            timeNum,
                            `${type}` as unitOfTime.DurationConstructor
                        );
                        message.channel.send(
                            `${
                                user.user.username
                            } is abonished to the depths of hell 2.0. Person shall be released: \`${releasement.date()}-${
                                releasement.month() + 1
                        ...
                        Clock.addDynamicReminder({
                            time: releasement,
                            func: () =>
                                this.releaseDoomed(afterAfterUser, Client),
                        });
                    });
            });
